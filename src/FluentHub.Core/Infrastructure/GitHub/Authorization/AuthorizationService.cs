...
namespace FluentHub.Core.Infrastructure.GitHub.Authorization
{
	public class AuthorizationService
	{
			...
			CancellationToken cancellationToken = default)
		{
			...
			while (DateTimeOffset.UtcNow < expiresAt)
			{
				...
				{
					progress?.Report(DeviceAuthorizationPollingStatus.Pending);
				}
				catch (DeviceAuthorizationSlowDownException ex)
				{
					interval = Math.Max(ex.Interval ?? interval + 5, interval + 5);
					progress?.Report(DeviceAuthorizationPollingStatus.SlowedDown);
				}
			}
	...
	public enum DeviceAuthorizationPollingStatus
	{
		Pending,
		SlowedDown,
	}
