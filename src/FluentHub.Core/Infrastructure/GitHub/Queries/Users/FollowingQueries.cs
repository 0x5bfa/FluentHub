...
{
	public partial class FollowingQueries
	{
		[GeneratedGraphQLOperation<GraphQLResult<User>>]
		private const string Query = """
			query Following($login: String!, $first: Int, $after: String, $last: Int, $before: String) {
			...
			CancellationToken cancellationToken = default)
		{
			...
			var response = await _gitHub.RunGraphQLAsync(
				...
				cancellationToken);
			var connection = response.Result?.Following
				?? throw new InvalidDataException("GitHub returned an incomplete following response.");
